...
@section('content')
    ...
    <form method="POST" action="{{ route('admin.blog.posts.store') }}" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <div class="row">
                <div class="col-sm-6">
                    ...
                    @foreach($tags as $tag)
                        <input type="checkbox" id="tagsExisting" name="tagsExisting[]" value="{{ $tag->id }}"> <span style="margin-right: 10px;">{{ $tag->name }}</span>
                    @endforeach
                </div>
        ...
        <div class="form-group">
            ...
            <textarea id="text" class="form-control {{ $errors->has('text') ? ' is-invalid' : '' }}" name="text" rows="10"></textarea>
            @if ($errors->has('text'))
