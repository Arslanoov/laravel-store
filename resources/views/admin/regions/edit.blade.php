...
@section('content')
    ...
    <form method="POST" action="{{ route('admin.regions.update', $region) }}">
        ...
        <div class="form-group">
            ...
            <select id="parent" class="form-control{{ $errors->has('parent') ? ' is-invalid' : '' }}" name="parent">
                ...
                @foreach ($parents as $parent)
                    <option value="{{ $parent->id }}"{{ $parent->id == old('parent', $region->parent_id) ? ' selected' : '' }}>
                        {{ $parent->name }}
                    </option>
                @endforeach;
            </select>
        ...
        <div class="form-group">
            ...
            <input id="name" class="form-control{{ $errors->has('name') ? ' is-invalid' : '' }}" name="name" value="{{ $region->name }}" required>
            @if ($errors->has('name'))
